...
export type ResultSheet = {
  ...
  /**
   * The structured record — see lib/student-profile.ts. Null when the student
   * has no StudentProfile row yet (pre-migration accounts before the backfill
   * runs). Admin audience only: a student's own copy of their result sheet has
   * no reason to restate their own address back to them.
   */
  profile: {
    phone: string | null;
    ...
    profile:
      audience === "admin"
        ? {
            ...
            stateRegion: student.profile?.stateRegion ?? null,
            ...
            nationality: student.profile?.nationality ?? null,
...
export function resultSheetToCsv(sheet: ResultSheet): string {
  ...
  const rows: string[][] = [
    ...
    ...(sheet.profile
      ? [
          ["Profile"],
          ["Phone", sheet.profile.phone ?? ""],
          ["WhatsApp", sheet.profile.whatsapp ?? ""],
          ["City", sheet.profile.city ?? ""],
          ["State/Region", sheet.profile.stateRegion ?? ""],
          ["Country", sheet.profile.country ?? ""],
          ["Date of birth", sheet.profile.dateOfBirth?.slice(0, 10) ?? ""],
          ["Gender", sheet.profile.gender ?? ""],
          ["Nationality", sheet.profile.nationality ?? ""],
          ["Guardian", [sheet.profile.guardianName, sheet.profile.guardianPhone].filter(Boolean).join(" — ")],
          ["Emergency contact", [sheet.profile.emergencyName, sheet.profile.emergencyPhone].filter(Boolean).join(" — ")],
          ["Tags", sheet.profile.tags.join(", ")],
          ["Segments", sheet.profile.segments.join(", ")],
          [],
        ]
      : []),
