...
export async function POST(req: Request) {
  ...
  const scheduledAt = typeof body?.scheduledAt === "string" ? parseTimeInput(body.scheduledAt) : new Date("invalid");
  ...
  const message = `${student.user.name ?? "A student"} requested ${formatWhen(scheduledAt)} for a private session.`;
  if (admins.length) {
