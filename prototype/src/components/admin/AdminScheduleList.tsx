...
/**
 * The admin schedule as filterable tables — every group session and every
 * private booking, plus the private-desk analytics. This is the "List" half of
 * /admin/schedule; the calendar is the default. Data is fetched by the parent
 * page and passed in, so the two views never double-fetch.
 */
...
export default function AdminScheduleList({
  ...
  const branches = useMemo(() => {
    ...
    groups.forEach((g) => {
      if (g.branchName) names.add(g.branchName);
    });
    privates.forEach((p) => {
      if (p.branchName) names.add(p.branchName);
    });
    return [...names].sort();
  }, [groups, privates]);
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {filteredGroups.map((item, index) => (
                    <tr key={`${item.date}-${item.cohort}-${index}`} className="border-t border-[var(--border)]">
                      <td className="p-3">{new Date(item.date).toLocaleDateString()}</td>
                      <td className="p-3">{item.cohort}</td>
                      <td className="p-3">
                        {item.startTime} - {item.endTime}
                      </td>
                      <td className="p-3">{item.tutorName ?? "Unassigned"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {filteredGroups.length === 0 && (
                <p className="p-6 text-sm text-[var(--muted)]">No group sessions match these filters.</p>
              )}
            </div>
          </section>
          {!levelFilter && (
          <section>
            ...
            {privateAnalytics && (
              <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
                ...
                <StatTile
                  label="No tutor assigned"
                  value={privateAnalytics.unassignedStudents}
                  warn={privateAnalytics.unassignedStudents > 0}
                />
                <StatTile
                  label="Paired, nothing booked"
                  value={privateAnalytics.noUpcomingSession}
                  warn={privateAnalytics.noUpcomingSession > 0}
                />
                <StatTile
                  label="Pending requests"
                  value={privateAnalytics.pendingRequests}
                  warn={privateAnalytics.pendingRequests > 0}
                />
                <StatTile label="Sessions next 7 days" value={privateAnalytics.upcoming7Days} />
                <StatTile
                  label="Cancelled this month"
                  value={privateAnalytics.cancelledThisMonth}
                  warn={privateAnalytics.cancelledThisMonth > 0}
                />
              </div>
            )}
            ...
            <div className="overflow-x-auto rounded-xl border border-[var(--border)]">
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {filteredPrivates.map((item) => (
                    <tr key={item.id} className="border-t border-[var(--border)] hover:bg-[var(--surface-alt)]">
                      <td
                        className="cursor-pointer p-3"
                        onClick={() => router.push(`/admin/schedule/private/${encodeURIComponent(item.studentId)}`)}
                      >
                      ...
                      >
                        {item.studentName}
                        <span className="block text-xs text-[var(--muted)]">{item.topic ?? "Private session"}</span>
                      </td>
                      <td
                        className="cursor-pointer p-3 capitalize"
                        onClick={() => router.push(`/admin/schedule/private/${encodeURIComponent(item.studentId)}`)}
                      >
                        {item.tutorName}
                      </td>
              ...
              {filteredPrivates.length === 0 && (
                <p className="p-6 text-sm text-[var(--muted)]">No private bookings match these filters.</p>
              )}
            </div>
          </section>
          )}
