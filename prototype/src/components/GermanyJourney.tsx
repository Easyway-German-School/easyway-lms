...
function StepBadge({ status }: { status: JourneyStep["status"] }) {
  ...
  return <span className="rounded-full bg-[var(--surface-alt)] px-3 py-1 text-xs font-semibold text-[var(--muted)]">Locked</span>;
}
    ...
    <section className="rounded-[28px] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-semibold text-[var(--foreground)]">Your path to Germany, mapped clearly.</h2>
          <p className="mt-2 text-sm leading-6 text-[var(--muted)]">Track each milestone from language learning to arrival.</p>
        </div>
      ...
      <div className="mt-6 rounded-[24px] border border-[var(--border)] bg-[#F8FBFF] p-5">
        <p className="text-sm font-semibold text-[var(--foreground-soft)]">You&apos;re {overallProgress}% closer to Germany 🇩🇪</p>
        <div className="mt-4 h-3 overflow-hidden rounded-full bg-[var(--surface-alt)]">
          <div className="h-3 rounded-full bg-gradient-to-r from-[#FF6600] to-[#FF9933]" style={{ width: `${overallProgress}%` }} />
        </div>
        <p className="mt-3 text-sm text-[var(--muted)]">Keep moving forward with your next milestone.</p>
      </div>
          ...
          return (
            <div
              ...
              className={`rounded-[24px] border p-4 shadow-sm transition ${
                isComplete ? "border-emerald-200 bg-emerald-50" : isCurrent ? "border-[#FF6600]/20 bg-[#FFF9F5]" : "border-[var(--border)] bg-[var(--surface-alt)]"
              }`}
            >
                ...
                <div className="flex items-center gap-4">
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-2xl text-lg shadow-sm ${
                      isComplete ? "bg-emerald-500 text-white" : isCurrent ? "bg-[#FF6600] text-white" : "bg-[var(--surface-alt)] text-[var(--muted)]"
                    }`}
                  >
                    {isComplete ? <CheckIcon className="h-5 w-5" strokeWidth={2.6} /> : step.icon}
                  </div>
                  <div>
                    <p className={`text-sm font-semibold ${isLocked ? "text-[var(--muted)]" : "text-[var(--foreground)]"}`}>{step.title}</p>
              ...
              <div className="mt-4">
                ...
                <p className="mt-2 text-xs font-medium text-[var(--muted)]">Progress {step.progress}%</p>
              </div>
            </div>
          );
        })}
