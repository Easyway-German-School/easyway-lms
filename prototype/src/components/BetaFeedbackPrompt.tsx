  ...
  return (
    <div className="fixed bottom-5 left-5 z-[70] w-[min(92vw,380px)] rounded-3xl border border-[var(--border-strong)] bg-[var(--surface)] p-5 shadow-2xl">
      ...
      <div className="flex items-center gap-3 pr-6">
        ...
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--accent)]">Becca wants to know</p>
          <h2 className="mt-1 text-lg font-bold text-[var(--foreground)]">Help shape the beta</h2>
        </div>
      </div>
      {sent ? <p className="mt-5 rounded-2xl bg-[var(--accent-soft)] px-4 py-3 text-sm font-semibold text-[var(--foreground)]">Thanks. Your note is with the Easyway team.</p> : (
        <form onSubmit={submit} className="mt-4 space-y-3">
          ...
          <textarea value={message} onChange={(event) => setMessage(event.target.value)} maxLength={2000} rows={4} placeholder="Tell us what would make Easyway better…" className="w-full resize-none rounded-2xl border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm text-[var(--foreground)] outline-none focus:border-[var(--accent)]" />
          ...
          <button type="submit" disabled={!message.trim() || sending} className="w-full rounded-2xl bg-[var(--accent-strong)] px-4 py-3 text-sm font-bold text-white disabled:opacity-50">{sending ? "Sending…" : "Send feedback"}</button>
        </form>
      )}
    </div>
  );
