...
export default function TandemPartner() {
  ...
  async function submitTurn() {
    if (scenarioIndex === null || !audioChunksRef.current.length) return;
    const scenario = scenarios[scenarioIndex];
    const yourLine = scenario.script[turnIndex].yourLine;
    setIsAnalyzing(true);
    try {
      ...
      form.append("expectedPhrase", yourLine);
      ...
      if (!response.ok) {
        setError(data?.error || "Unable to reach the voice practice API.");
        return;
      }
      ...
      const youMessage: Message = {
        id: `you-${scenarioIndex}-${turnIndex}`,
        speaker: "you",
        text: data.transcription || yourLine,
        coaching,
      };
      ...
      setError("Unable to reach the voice practice API.");
    } finally {
              ...
              {currentYourLine ? (
                <div className="flex flex-col items-end gap-2">
                  ...
                  <button
                    ...
                    disabled={isAnalyzing || !micSupported}
                    className={`inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-bold text-white transition disabled:cursor-not-allowed disabled:opacity-50 ${isRecording ? "bg-red-600" : "bg-emerald-500 hover:bg-emerald-600"}`}
                  >
                    <MicIcon className="h-4 w-4" />
                    {isAnalyzing ? "Coaching your line…" : isRecording ? `Stop · 0:${String(recordingSeconds).padStart(2, "0")}` : "Say your line"}
                  </button>
                </div>
              ) : null}
