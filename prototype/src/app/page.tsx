...
export default async function HomePage() {
  ...
  if (!session?.user?.id) {
    // Pre-account marketing page: a fixed light gradient, not the app's
    // --background. Scoped to theme-light so a visitor whose browser has
    // Nacht/Dämmerung saved from a previous session doesn't get this page's
    // hardcoded dark text sitting on a card that just went dark underneath it.
    return (
      <main className="theme-light min-h-screen bg-[linear-gradient(135deg,_#f5f5f5_0%,_#fffbf8_100%)] px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-6xl flex-col overflow-hidden rounded-[32px] border border-[var(--border)] bg-[var(--surface)] shadow-[0_30px_80px_-24px_rgba(13,124,126,0.35)] lg:flex-row">
          ...
          <div className="flex-1 p-8 sm:p-10 lg:p-12">
            <div className="rounded-[24px] border border-[var(--border)] bg-[#F8FBFF] p-6 shadow-sm">
              ...
              <ul className="mt-4 space-y-3 text-sm text-[var(--muted)]">
                {[
                  ...
                  <li key={feature.title} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-4 py-4 shadow-sm">
                    <p className="text-sm font-semibold text-[#333333]">{feature.title}</p>
                    <p className="mt-1 text-sm text-[var(--muted)]">{feature.detail}</p>
                  </li>
                ))}
              </ul>
            </div>
            <div className="mt-8 rounded-[24px] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[var(--muted)]">Why Easyway</p>
        ...
        {process.env.NODE_ENV === 'development' ? (
          <div className="mx-auto mt-8 max-w-6xl rounded-[32px] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[0_30px_80px_-24px_rgba(0,48,135,0.35)]">
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[var(--muted)]">Developer Navigation</p>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                <Link key={item.href} href={item.href} className="rounded-2xl border border-[var(--border)] bg-[#F8FBFF] px-4 py-3 text-sm font-medium text-[var(--foreground-soft)] transition hover:border-blue-200 hover:bg-[var(--surface)]">
                  {item.label}
                </Link>
              ))}
            </div>
