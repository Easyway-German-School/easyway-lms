...
type Usage = {
  ...
  limits: { standard: Record<string, number>; private: Record<string, number> };
        ...
        {loading ? <p className="text-sm text-[var(--muted)]">Loading live usage…</p> : usage && (
          <>
            ...
            <section className="grid gap-6 lg:grid-cols-2">
              ...
              <div className="rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-5"><h2 className="text-lg font-semibold">Private membership allowance</h2><p className="mt-2 text-sm text-[var(--muted)]">Private students receive additional daily practice capacity.</p><div className="mt-4 grid grid-cols-3 gap-3 text-center text-sm"><div><strong className="block text-xl">{usage.limits.private.essay}</strong><span className="text-[var(--muted)]">essays</span></div><div><strong className="block text-xl">{usage.limits.private.pronunciation}</strong><span className="text-[var(--muted)]">speaking</span></div><div><strong className="block text-xl">{usage.limits.private.missionPractice}</strong><span className="text-[var(--muted)]">missions</span></div></div></div>
            </section>
