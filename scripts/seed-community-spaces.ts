...
async function main() {
  ...
  console.log(`📍 Found ${branches.length} branches and ${levels.length} levels:\n`);
  ...
  // Create or update spaces for each branch × level combination
  for (const branch of branches) {
      ...
      try {
        const space = await prisma.space.upsert({
          ...
          create: {
            branchId: branch.id,
            level,
            name: `${branch.name} - Level ${level}`,
            description: `Community space for ${branch.name} learners at level ${level}. Discuss lessons, ask questions, and collaborate with peers.`
          }
        });
        ...
        spacesCreated++;
        console.log(`✅ ${branch.name} - Level ${level}`);
      } catch (e) {
        // Space already exists
        if ((e as any).code === "P2002") {
          skipped++;
          console.log(`⏭️  ${branch.name} - Level ${level} (already exists)`);
        } else {
